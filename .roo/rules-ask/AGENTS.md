...
- 该项目是**单个 `.csproj`** Windows Forms 应用，目标 .NET Framework 4.8，仅 x64。无 NuGet 依赖。
...
- DPI 缩放：[`App.config`](NoFences/App.config:7) 设置为 `PerMonitorV2`。`FenceInfo` 中的 `Width`/`Height` 是原样存储的 DPI 缩放值；`TitleHeight` 以逻辑像素存储，通过 [`LogicalToDeviceUnits`](NoFences/FenceWindow.cs:64) 进行缩放。
